...
html = f'''<style>
  ...
  *{{ box-sizing:border-box; }}
  ...
  /* 소리 재생 중: 버튼 색 활성화(파랑→주황) + 음파 줄이 늘었다 줄었다 */
  .prompt__speaker.speaking {{ background:var(--couple);
    box-shadow:0 0 0 0.6cqmin var(--couple-soft), 0 0.6cqmin 1.4cqmin rgba(238,151,64,.42); }}
  .prompt__speaker svg .wave {{ transform-box:fill-box; transform-origin:left center; }}
  .prompt__speaker.speaking .wave1 {{ animation:spkWave .8s ease-in-out infinite; }}
  .prompt__speaker.speaking .wave2 {{ animation:spkWave .8s ease-in-out infinite .18s; }}
  @keyframes spkWave {{ 0%,100%{{ transform:scaleX(.4); opacity:.45; }} 50%{{ transform:scaleX(1); opacity:1; }} }}
  ...
  .now__card {{ background:var(--surface); border:0.5cqmin solid var(--line-strong); border-radius:3.8cqmin;
    ...
    align-items:center; justify-content:center; gap:1.2cqmin; box-shadow:0 8px 18px rgba(56,68,79,.08); cursor:pointer; }}
  ...
  .link__tok {{ min-width:7.2cqmin; height:7.2cqmin; padding:0 1.2cqmin; border-radius:50%;
    ...
    font-family:var(--display); font-size:4.35cqmin; display:flex; align-items:center; justify-content:center; cursor:pointer; }}
  ...
  /* armed: 카드가 빈 칸에 걸친 상태 — 점선→실선 전환으로만 표시(노란 글로우·맥동 없음) */
  ...
  .opt {{ font-family:var(--display); background:var(--surface); border:0.5cqmin solid var(--card-edge);
    ...
    <div class="wrap">
      ...
      <section class="stage" id="stage" aria-label="지금 차례">
        <div class="now">
          ...
          <div class="link"><span class="link__bar"></span><span class="link__tok" id="linkTok" role="button" tabindex="0" aria-label="글자 듣기"></span><span class="link__bar"></span></div>
          <div class="drop" id="drop"><span class="drop__q">?</span></div>
        </div>
        <div class="prompt" id="prompt">
          <button class="prompt__speaker" type="button" aria-label="다시 듣기"><svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M3 9.2H6.6L11.5 5V19L6.6 14.8H3Z" fill="#fff"/><path class="wave wave1" d="M15 8.6a4.6 4.6 0 0 1 0 6.8" stroke="#fff" stroke-width="1.9" stroke-linecap="round"/><path class="wave wave2" d="M17.7 6a8.2 8.2 0 0 1 0 12" stroke="#fff" stroke-width="1.9" stroke-linecap="round"/></svg></button>
...
<script>
  ...
  function attachCard(card, word) {{
    ...
    card.addEventListener('pointerdown', e => {{
      ...
      if (!reduceMotion) {{                 // 누르면 살짝 떠올라 멈춤(이 팝업 들림을 드래그 내내 그대로 유지)
    ...
    card.addEventListener('pointermove', e => {{
      ...
      if (dragging) {{
        card.style.transform = reduceMotion
          ...
          : 'translate(' + dx + 'px, calc(-2.4cqmin + ' + dy + 'px))';  // 누름 팝업 들림(-2.4cqmin) 그대로 유지 — 배율 변화 없이 추종
  ...
  // 프롬프트 문장("…로 시작하는 말은?") 발화 동안 스피커 버튼을 활성 상태로(색+음파 애니메이션)
  const promptSpeaker = document.querySelector('.prompt__speaker');
  function bindSpeaking(u) {{
    if (!promptSpeaker) return;
    u.addEventListener('start', () => promptSpeaker.classList.add('speaking'));
    const off = () => promptSpeaker.classList.remove('speaking');
  ...
  function speakRound(word, sentence) {{
    ...
    // 스피커 활성은 단어 시작부터 문장 끝까지 한 번만 켜고 끔 — 사이 0.3초 무음에도 유지
    const spkOff = () => {{ if (promptSpeaker) promptSpeaker.classList.remove('speaking'); }};
    const u1 = _utter(word);
    u1.onstart = () => {{ if (seq === _speakSeq && promptSpeaker) promptSpeaker.classList.add('speaking'); }};
    u1.onerror = spkOff;
    u1.onend = () => {{
      if (seq !== _speakSeq) {{ spkOff(); return; }}   // 그 사이 다른 발화가 시작됐으면 중단
      setTimeout(() => {{
        if (seq !== _speakSeq) {{ spkOff(); return; }}
        const u2 = _utter(sentence); u2.onend = spkOff; u2.onerror = spkOff; speechSynthesis.speak(u2);
      }}, 300);
    }};
  ...
  promptSpeaker.addEventListener('click', () => {{
    ...
    _speakSeq++;
    speechSynthesis.cancel();
    const u = _utter($('promptText').textContent);
    bindSpeaking(u);
    speechSynthesis.speak(u);
  }});
