...
1. **✅ 그림(pic) 매핑 — 결정됨: 폴백(graceful degradation).** 새 단어 756개 중 258개만 기존 그림 매핑 보유(옛 `v2/words.json`의 `word→pic`), 498개는 그림 없음. → **그림 있으면 표시, 없으면 글자만** 보여준다. 매핑 테이블(word→pic, 258개)은 `data/`로 옮겨 유지하고, `ui.js`는 매핑 없는 단어를 텍스트 카드로 렌더한다. 신규 단어 그림 소싱은 추후(Phase 2 이후) 별도 작업.
